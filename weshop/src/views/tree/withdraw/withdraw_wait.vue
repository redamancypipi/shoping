<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column prop="userId" label="提现用户ID" width="200" />
      ...
      <el-table-column prop="gmtCreate" label="提现时间" width="250" />
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="scope">
          <el-button type="danger" size="mini" @click="editWithdraw(scope.row.id)">同意</el-button>
          <el-button type="primary" size="mini" @click="RefuseWithdraw(scope.row.id,scope.row.userId,scope.row.money)">拒绝</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    // 拒绝提现，先还钱，再改状态
    RefuseWithdraw(id, userId, money) {
      withDraw.payBackMoney(userId, money)
        .then(res => {
          console.log(res)
          withDraw.refuseWithdraw(id)
            .then(res => {
              console.log(res)
              this.getList()
            })
